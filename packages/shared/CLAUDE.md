...
| `src/column-filter.ts` | The filter LANGUAGE: `isListExpression` decides list-vs-plain-text, then an expression parses into `FilterToken[]` and matches in memory. **The specification** every backend must agree with. A bare value's meaning depends on `defaultSubstring`, which the caller passes in. `matchesColumnFilter` takes an optional clock (`now`) so a relative comparison is testable without waiting for real time to pass. |
...
| `src/compare-cell.ts`  | What a `>=`/`<=`/`>`/`<` comparison means per `ColumnType`: `date` compares by calendar date, `datetime` at full precision, `number` numerically, anything else as lowercased text. Normalises a zoned value to UTC so this and the SQL sibling can't drift. Also `isValidBound`. |
| `src/relative-date.ts` | `resolveDateTerm`: relative bounds for a date comparison — `-7d`/`-2w`/`-3m`/`-1y`, `today`, `wtd`, `mtd`, `qtd`, `ytd`. Month/year offsets clamp to the end of a short month (31 Mar − 1m = 28 Feb). |
| `src/filter-sql.ts`    | The same tokens as a SQL `WHERE` fragment, comparisons included, via `date()` / `datetime()` / a `numberExpr()` validator / text — cross-tested against the matcher.                                |
| `src/filter-range.ts`  | `readRange` / `composeRange`: pulls a from/to comparison pair out of a filter string (for a picker UI) while preserving every other token, and writes it back.                                       |
