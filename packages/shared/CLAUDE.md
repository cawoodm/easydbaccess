...
| `src/column-filter.ts` | The filter LANGUAGE: `isListExpression` decides list-vs-plain-text, then an expression parses into `FilterToken[]` and matches in memory. **The specification** every backend must agree with. A bare value's meaning depends on `defaultSubstring`, which the caller passes in. |
