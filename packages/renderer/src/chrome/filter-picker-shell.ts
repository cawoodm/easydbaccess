...
/** Result of a filter picker: null (dismissed), cleared, or fallback. */
type FilterPickerResult = null | { clear: true } | { fallback: true };
...
export abstract class FilterPickerShell extends LitElement {
  ...
  /**
   * The column label shown in the picker's header. Set by a subclass in its
   * `open()` method and passed to `renderShell`.
   */
