...
| `src/plugins/` | Built-in plugins. **Each one IS a plugin** — same contract as URL-loaded modules. Current roster: `new-table-button`, `csv-import`, `json-import`, `sql-import` (+ the pure `sql-parse`), `csv-export`, `dump-export`, `sql-export` (+ `projection-sql`), `gist-sync`, `server-sync` (+ `server-sync-core`), `auto-sync`, `cell-color`, `cell-image`, `cell-link`, `cell-date`, `cell-datetime`, `cell-boolean`, `auto-renderer`, `import-data`, `table-copy`, `views`, `settings`, `projection` (+ `projection-compute`, `projection-collection`, `projection-create`), `electron-db` (registers nothing outside the Electron build). (The Plugin Manager button is **core**, not a plugin — see `app-shell.ts`. The URL-loadable demo plugins under `public/plugins/` — `header-clock`, `cell-image-url`, `cell-email` — are separate from these bundled built-ins.) |
