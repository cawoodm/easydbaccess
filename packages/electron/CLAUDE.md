...
| `src/db-files.ts`            | The store singleton (open / close / switch, remembered path) and the Open / Save As file operations, including the OS dialogs. Owns `readLocationConfig` / `patchLocationConfig` — `db-location.json` carries three unrelated things, so every writer merges rather than overwrites. |
| `src/db-folder.ts`           | The workspace folder: the remembered path, the directory picker, a read-only scan of every `.edb` in it, and writing a new one. The desktop half of what the browser does with a `FileSystemDirectoryHandle` — see `docs/tech/ELECTRON.md`. |
