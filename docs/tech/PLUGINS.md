...
| `electron-db`       | ui            |       | Open, Save As and Import for `.db` / `.edb` files, as palette commands under "File" and "Data". **Desktop-only, and silent about it:** `init()` returns immediately when `window.easydb?.db` is absent, so the browser build gets no command and no error. Open only accepts a file this app wrote and offers Browse or Convert for a foreign one; Import previews first and asks Overwrite / Rename / Skip per colliding table. The footer "Database" button it used to register went in v0.0.491 — see `electron-folder` below. See `STORAGE.md` and `ELECTRON.md`. | `registerCommand`, `registerDropHandler`                           |
| `electron-folder`   | ui            |       | The workspace folder on the desktop: Connect ▸ Local Data, "Connect workspace folder…" and "Sync workspace folder". **The same connector id, label and dialog the browser's `edb-file` registers**, so the Connect menu is identical in both builds. It writes the device-local folder index the workspace selector already reads, and installs the `FileWorkspaceBackend` that opens another file and creates a new one. Loaded by `electron-db`, not by the plugin loader. | `registerConnector`, `registerCommand`                             |
