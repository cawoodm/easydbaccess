...
2. **Aggregated buttons** — every header and footer button
   ...
   _New Table_, _Import_, _Export_, _Gist_, _Sync_, _Settings_, etc. are
...
open panel — table windows _and_ view-instance windows — via the panel
...
Which window gets which slot is a second, separate question, answered by
`nearestSlots` (also pure): every window takes the slot NEAREST to where it
already is. The slots come out in reading order and the panels arrive in
stacking order, so handing them out in step shuffled any layout the user had
built — tile a hand-made grid and each window landed by how recently it was
touched. `nearestSlots` picks the closest free pair over and over, then swaps
any two windows that are better the other way round. Cascade is the one
arrangement that keeps the stacking order, because a cascade IS a stack.
...
live pan/zoom transform). They set inline geometry, which no panel callback
reports, and then ask both window managers to persist it — see
`persistArrangement`.
